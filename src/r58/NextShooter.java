...
public class NextShooter {
    ...
    final int THEIRSHIPSPASTTHREE = 7;
    private boolean noMiss=true;
    private int misses;
    ...
    public void addShot(Position p,boolean hit){
        ...
        if (turnCount<=4) maps[p.x][p.y][0]++;
        if (hit) maps[p.x][p.y][1]=1;
        else {
            maps[p.x][p.y][1]=-1;
    ...
    public void shipWrecked(Position p, int length){
        ...
        for (int i = 0; i < length; i++) {
            if (p.y+i>=boardY || maps[p.x][p.y+i][1]!=1) up=false;
            if (p.y-i<0 || maps[p.x][p.y-i][1]!=1) down=false;
            if (p.x-i<0 || maps[p.x-i][p.y][1]!=1) left=false;
            if (p.x+i>=boardX || maps[p.x+i][p.y][1]!=1) right=false;
        }
        if (up){
            if(testing)System.out.println("Removing ship up");
            for (int i = 0; i < length; i++) {
                maps[p.x][p.y+i][1]=-1;
                maps[p.x][p.y+i][THEIRPLACEMENTS]+=10;
                maps[p.x][p.y+i][THEIRSHIPSPASTTHREE]++;
            }
        }
        if (down){
            ...
            for (int i = 0; i < length; i++) {
                maps[p.x][p.y-i][1]=-1;
                maps[p.x][p.y-i][THEIRPLACEMENTS]+=10;     //The cooldown value. Higher value - longer memory
                maps[p.x][p.y-i][THEIRSHIPSPASTTHREE]++;
            }
        }
        if (left){
            ...
            for (int i = 0; i < length; i++) {
                maps[p.x-i][p.y][1]=-1;
                maps[p.x-i][p.y][THEIRPLACEMENTS]+=10;
                maps[p.x-i][p.y][THEIRSHIPSPASTTHREE]++;
            }
        }
        if (right){
    ...
    public void fillGridList(){
        ...
        if (gridPositions.size()>50)System.out.println("Too many grid positions");
}
    public Position shootInGrid(int gridCase){
        //if (checkIdentShips()) return shootSamePosition();
        if (gridCase==0) return shootInGridNormal();
    ...
    public Position shootInGridLostFour(){
        ...
        for (Position p : gridPositions) {
            if (maps[shot.x][shot.y][SHIPCOMBOS]+Math.min((maps[shot.x][shot.y][6]/5),5)
                    <maps[p.x][p.y][SHIPCOMBOS]+Math.min((maps[p.x][p.y][6]/5),5)) shot=p;
        }
    ...
    public Position huntOtherWay(){
        ...
        if (gridPositions.contains(prospect)) gridPositions.remove(prospect);
        return prospect;
    }
    ...
    public Position hunt(Position hit){
        ...
        if (!bingo) shot = huntLocations.remove((int)(Math.random()*huntLocations.size()));
        shotsFired.add(shot);
        return shot;
    }
    ...
    public boolean checkForMoreTargets(){
        for (int y = boardY-1; y >= 0; y--) {             
                for (int x = 0; x < boardX; x++) {          
                    if(maps[x][y][1]==1) return true;       // Returns true if the map has a
...
public void doComboMap(){
    ...
    if(testing)System.out.println("fill temp maps with combos");
                ...
                if(testing)System.out.println("Sum the temp maps");
...
public void updateComboMap(Position p){
        ...
        int[][] map = new int[boardX][boardY];
                //Write from x=0;
                        ...
                        for (int j = 0; j < ship.size(); j++) {
                            if (!vertical && (p.x+j>boardX-1 || p.x+j<0 || maps[p.x+j][p.y][1]==-1)) fits=false;
                            if (vertical && (p.y+j>9  || p.y+j<0|| maps[p.x][p.y+j][1]==-1)) fits=false;
                        }
                        if (fits){
                            for (int j = 0; j < ship.size(); j++) { 
                            if (!vertical && maps[p.x+j][p.y][1]!=1) map[p.x+j][p.y]++;
                            if (vertical && maps[p.x][p.y+j][1]!=1) map[p.x][p.y+j]++;
                        }
                    }
    ...
    public int [][] possibleShipCombosMap(Ship ship, boolean vertical){
        ...
        for (int y = boardY-1; y >= 0; y--) {              //Write from top -> 9
                for (int x = 0; x < boardX; x++) {          //Write from x=0;
                        ...
                        boolean fits=true;
                        for (int j = 0; j < ship.size(); j++) {
                            if (!vertical && (x+j>boardX-1 || maps[x+j][y][1]==-1)) fits=false;
                            if (vertical && (y+j>boardY-1 || maps[x][y+j][1]==-1)) fits=false;
                        }
                        if (fits){
                            for (int j = 0; j < ship.size(); j++) { 
                            if (!vertical && maps[x+j][y][1]!=1) map[x+j][y]++;
                            if (vertical && maps[x][y+j][1]!=1) map[x][y+j]++;
                        }
                    }
                }
            }
    ...
    public void coolDownPlacement(){
        for (int y = boardY-1; y >=0; y--) {
            for (int x = 0; x < boardX; x++) {
                if (maps[x][y][THEIRPLACEMENTS]>=0)maps[x][y][THEIRPLACEMENTS]-=5;
                if (maps[x][y][THEIRPLACEMENTS]>=100)maps[x][y][THEIRPLACEMENTS]=100;
                if (maps[x][y][THEIRPLACEMENTS]<0)maps[x][y][THEIRPLACEMENTS]=0;
            }
        }
    }
